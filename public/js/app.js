...
let sessionReady = false; // サーバー側sessions Mapへの復元が完了したか
...
function switchSettingsTab(name) {
  ...
  if (name === 'friends') ensureSessionReady(() => socket.emit('getFriends'));
}

// サーバー側sessions Mapへの復元(restoreSession)が完了するまで待ってから
// callbackを実行する。ページ読み込み直後はDBレイテンシ等でまだ復元が
// 完了していない場合があり、その状態でフレンド操作を送ると「ログインが
// 必要です」と誤って拒否されてしまうため。
function ensureSessionReady(callback) {
  if (sessionReady || !myPlayerId) { callback(); return; }
  let fired = false;
  const fire = () => { if (!fired) { fired = true; callback(); } };
  socket.once('sessionRestored', fire);
  socket.emit('restoreSession', { playerId: myPlayerId });
  // sessionRestoredが何らかの理由で届かない場合のフォールバック
  // (届けば即座にfire()、届かなくても操作自体は最終的に送られる)
...
function openFriendInviteModal() {
  ...
  ensureSessionReady(() => socket.emit('getFriends'));
}
...
function bindEvents() {
  ...
  els.avatarGrid.addEventListener('click', e => {
    ...
    if (!btn) return;
    ensureSessionReady(() => socket.emit('setAvatar', { avatar: btn.dataset.avatar }));
  });
  ...
  els.formChangeUsername.addEventListener('submit', e => {
    ...
    btn.disabled = true;
    btn.textContent = '変更中...';
    ensureSessionReady(() => socket.emit('changeUsername', { newUsername, password }));
  });
  ...
  els.formChangePassword.addEventListener('submit', e => {
    ...
    btn.disabled = true;
    btn.textContent = '変更中...';
    ensureSessionReady(() => socket.emit('changePassword', { currentPassword, newPassword }));
  });
  ...
  els.formDeleteAccount.addEventListener('submit', e => {
    ...
    btn.disabled = true;
    btn.textContent = '削除中...';
    ensureSessionReady(() => socket.emit('deleteAccount', { password }));
  });
  ...
  els.formAddFriend.addEventListener('submit', e => {
    ...
    if (!username) return;
    ensureSessionReady(() => socket.emit('sendFriendRequest', { username }));
  });
  ...
  els.friendRequestsList.addEventListener('click', e => {
    ...
    if (btn.dataset.action === 'accept') ensureSessionReady(() => socket.emit('respondFriendRequest', { requesterId, accept: true }));
    else if (btn.dataset.action === 'reject') ensureSessionReady(() => socket.emit('respondFriendRequest', { requesterId, accept: false }));
  });
  ...
  els.friendsListEl.addEventListener('click', e => {
    ...
    if (btn.dataset.action === 'remove') {
      if (confirm('フレンドを削除しますか？')) ensureSessionReady(() => socket.emit('removeFriend', { friendId: btn.dataset.playerId }));
    }
  });
  ...
  els.inviteFriendsList.addEventListener('click', e => {
    ...
    btn.disabled = true;
    ensureSessionReady(() => socket.emit('inviteFriendToRoom', { friendId: btn.dataset.playerId }));
  });
